...
The single-request comparison path uses eager execution only.
"""
...
class QSAHiSparseSingleRequest:
    """Single-request eager comparison and shared request-local byte operations."""
    ...
    def __init__(self, runner, mode):
        ...
        if getattr(self.pool, "qsa_hisparse", None) is not None:
        ...
        if (
            type(self.full) is not MHATokenToKVPool
        ...
        self.mamba_ptrs, self.mamba_sizes, _ = (
    ...
    def record(self, event, **extra):
        ...
        row = {
            ...
            "host_slab_ptr": None
            if self.host_slab is None
            else self.host_slab.data_ptr(),
            ...
            "mamba_storage_unchanged": self.mamba_ptrs
            == self.mamba_pool.get_contiguous_buf_infos()[0],
    ...
    def begin_batch(self, batch):
        ...
        if self.owner is None:
            ...
            if self.raw_released:
                ...
                if self.full.k_data_ptrs.cpu().tolist() != [
                    k.data_ptr() for k in self.full.k_buffer
                ]:
                    raise AssertionError("restored K pointer metadata differs")
                if self.full.v_data_ptrs.cpu().tolist() != [
                    v.data_ptr() for v in self.full.v_buffer
                ]:
        ...
        if self.offloaded and not decode:
            raise RuntimeError(
                "QSA V3 cannot retract an offloaded request into prefill"
            )
    ...
    def handoff(self, prompt_len):
        if (
            ...
            or self.host_slab is None
        ):
            raise RuntimeError(
                "QSA V3 handoff requires an unclaimed host slab and active lease"
            )
        ...
        for li, lid in enumerate(self.layer_ids):
            ...
            if tail:
                ring_k[1 : 1 + tail].copy_(
                    k.view(torch.uint8).index_select(0, slots[-tail:]).view(k.dtype)
                )
                ring_v[1 : 1 + tail].copy_(
                    v.view(torch.uint8).index_select(0, slots[-tail:]).view(v.dtype)
                )
            ...
            self.record(
                "handoff_layer",
                layer=lid,
                source_bytes=prompt_len * 512,
                d2h_bytes=blocks * self.ITEM,
                d2h_event_ms=copy_ms,
        ...
        self.unpacked = torch.empty(
            (2, 2048, 1, 256), dtype=torch.uint8, device=self.device
        )
        self.compact = torch.zeros(
            (2, 2052, 1, 256), dtype=torch.uint8, device=self.device
        )
        self.compact_table = torch.zeros(
            (1, self.capacity), dtype=torch.int32, device=self.device
        )
        ...
        self.record(
            ...
            allocated_drop_bytes=allocated_before
            - torch.cuda.memory_allocated(self.device),
            pending_tail=tail,
            logical_lease_preserved=True,
        )
    ...
    def make_state(self):
        ...
        return {
            "hot": torch.zeros(
                (physical, self.ITEM), dtype=torch.uint8, device=self.device
            ),
            "tokens": torch.full(
                (1, physical), -1, dtype=torch.int32, device=self.device
            ),
            "lru": torch.arange(self.HOT, dtype=torch.int16, device=self.device)[None],
            "host_locs": torch.arange(
                (self.capacity + 3) // 4, dtype=torch.int64, device=self.device
            )[None],
            ...
            "miss_src": torch.empty(
                (1, self.TOPK), dtype=torch.int64, device=self.device
            ),
            "miss_dst": torch.empty(
                (1, self.TOPK), dtype=torch.int32, device=self.device
            ),
    ...
    def selected(self, layer, raw_indices):
        ...
        with operations_nvtx_range("qsa.hot_gather"):
            torch.index_select(
                state["hot"], 0, state["out"].reshape(-1).long(), out=self.gathered
            )
        ...
        self.finish_selected(
            layer, raw_indices, blocks, self.unpacked, state["miss_count"]
        )
        return (
    ...
    def finish_selected(self, layer, raw_indices, blocks, unpacked, miss_count):
        ...
        with operations_nvtx_range("qsa.compact_mapping"):
            ...
            if tail:
                start = 1 + selected
                self.compact[0, start : start + tail].copy_(
                    self.full.k_buffer[li][1 : 1 + tail].view(torch.uint8)
                )
                self.compact[1, start : start + tail].copy_(
                    self.full.v_buffer[li][1 : 1 + tail].view(torch.uint8)
                )
            valid = raw_indices[0, : selected + tail].long()
            base = getattr(self, "compact_base", 0)
            self.compact_table[0, valid] = torch.arange(
                1 + base,
                1 + selected + tail + base,
                dtype=torch.int32,
                device=self.device,
            )
    ...
    def check_selected(
        self, layer, raw_indices, blocks, miss_count, *, compact=None, mapping=None
    ):
        """Independent host-byte oracle, also callable on post-replay snapshots."""
        check = self.strict and (
            self.decode_steps in (1, 2, 3, 384, 767) or self.seq_len % 4 == 0
        )
        if check:
            li = self.pool._transfer_full_attention_id(layer.layer_id)
            state = self.states[li]
            ...
            if len(torch.unique(ids)) != selected // 4 or (
                ids.numel()
                and (int(ids.min()) < 0 or int(ids.max()) >= self.seq_len // 4)
            ):
                raise AssertionError("invalid selected C4 set")
            if not torch.equal(
                raw_indices[0, :selected].cpu().reshape(-1, 4),
                ids[:, None] * 4 + torch.arange(4),
            ):
            ...
            ek, ev = (
                expected[:, :1024].reshape(-1, 1, 256),
                expected[:, 1024:].reshape(-1, 1, 256),
            )
            if not torch.equal(
                compact[0, 1 : 1 + selected].cpu(), ek
            ) or not torch.equal(compact[1, 1 : 1 + selected].cpu(), ev):
                raise AssertionError("selected unpack K/V bytes differ")
            if not torch.equal(
                raw_indices[0, selected:].cpu(),
                torch.cat(
                    (
                        torch.arange(
                            self.seq_len - tail, self.seq_len, dtype=torch.int32
                        ),
            ...
            for plane, source in (
                (0, self.full.k_buffer[li]),
                (1, self.full.v_buffer[li]),
            ):
                if not torch.equal(
                    compact[plane, 1 + selected : 1 + selected + tail].cpu(),
                    source[1 : 1 + tail].view(torch.uint8).cpu(),
                ):
                    raise AssertionError("pending tail bytes differ")
            if not torch.equal(
                mapping.cpu(),
                torch.arange(1 + base, 1 + selected + tail + base, dtype=torch.int32),
            ):
                raise AssertionError("compact physical mapping differs")
            self.record(
                "selected_check",
                layer=layer.layer_id,
                bytes_checked=selected * 512,
                tail=tail,
                tail_bytes_checked=tail * 512,
                mapping_checked=True,
                page_boundary=self.seq_len % 64 == 0,
                latest_writeback_event_ms=(
                    state["copy_begin"].elapsed_time(state["done"])
                    if state.get("copy_begin") is not None
                    else None
                ),
                writeback_bytes=state["writeback_bytes"],
    ...
    def release(self, req_idx, rid):
        ...
        self.record(
            ...
            rid=rid,
            pending_events=0,
            terminal_wall_ms=terminal_ms,
            copy_drain_wall_ms=drain_ms,
        )
        return lease
    ...
    def capture_decode(
        ...
        row = {
            ...
            "indices": raw_indices.detach().cpu(),
            "k_scale": k_scale,
            "v_scale": v_scale,
            ...
            "q_stride": q.stride(),
            "k_shape": packed_k.shape,
            "k_stride": packed_k.stride(),
            "v_shape": packed_v.shape,
            "v_stride": packed_v.stride(),
        }
        ...
        if layer.layer_id == self.layer_ids[-1]:
            torch.save(
                ...
                self.capture_dir
                / f"gen-{self.generation}-step-{self.decode_steps:04d}-rank-{self.rank}.pt",
            )
            self.capture_layers = {}
