...
class QSAHiSparseSlots:
    def __init__(self, staging_tokens: int, page_size: int, max_requests: int):
        if (
            ...
            or page_size % 4
            or staging_tokens <= 0
            or staging_tokens % page_size
            or not 1 <= max_requests <= 8
        ):
            raise ValueError(
                "QSA P2 needs page-aligned C4 staging and at most eight leases"
            )
    ...
    def acquire(self, req_pool_idx: int, generation: int, rid: str):
        if req_pool_idx <= 0 or generation <= 0 or not rid:
            raise ValueError(
                "QSA lease requires a real request slot, generation and ID"
            )
    ...
    def snapshot(self):
        return {
            ...
            "staging_available_tokens": self.staging_tokens
            if self.prefill_owner is None
            else 0,
            "staging_owner": None
            if self.prefill_owner is None
            else self.prefill_owner.req_pool_idx,
