# QSA HiSparse experiment archive
...
This directory preserves the design notes, bounded validation harnesses,
profiles, and reduced results from the 2026-09-11 source revision. These are
historical measurements, not validation of the current source tree. Sanitized SVG figures derived from NSYS are included; raw service logs,
...
## Reproduce the historical revision

The complete original runtime is retained in this repository's Git history at
`5f8ae43640404eaee4c645d8cad2d0ef6e7dc6b0`. Use a separate worktree at that
revision to reproduce these experiments. The old 13 patch files remain available
in artifact commit `e1ac504`; they are not part of the current install workflow.
For the maintained implementation, start at the [project README](../README.md).
...
Apache-2.0. Original source headers and commit attribution are retained in Git history.
