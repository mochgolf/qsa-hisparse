...
from sglang.srt.mem_cache.qsa_hisparse.single_request import (
    QSAHiSparseSingleRequest,
    pack_c4,
    stage_short_prefix,
    unpack_index,
...
class TestQSAHiSparseSingleRequest(unittest.TestCase):
    def test_startup_slab_reuse_and_appended_writeback(self):
        ...
        full = Full(
            k_buffer=[torch.zeros((2056, 1, 256), dtype=torch.uint8)],
        ...
        with (
            ...
            patch(
                "sglang.srt.mem_cache.qsa_hisparse.single_request.validate_configuration"
            ),
            patch(
            ...
            patch.object(QSAHiSparseSingleRequest, "record") as record,
        ):
            resident = QSAHiSparseSingleRequest(runner, "resident")
            self.assertIsNone(resident.host_slab)
            self.assertEqual(allocations, [])
            adapter = QSAHiSparseSingleRequest(runner, "offload")
            self.assertEqual(
                allocations,
                [((12, 65536, 2048), {"dtype": torch.uint8, "pin_memory": True})],
            )
            self.assertEqual(record.call_args.kwargs["allocation_phase"], "startup")
            with (
                patch.object(torch, "empty", side_effect=RuntimeError("pin failure")),
                self.assertRaisesRegex(RuntimeError, "pin failure"),
            ):
                QSAHiSparseSingleRequest(runner, "offload")
        ...
        def check_ledger(event, **extra):
            ...
            with (
                ...
                QSAHiSparseSingleRequest.record(adapter, event, **extra)
        ...
        runner.req_to_token_pool.mamba_allocator = SimpleNamespace(
            available_size=lambda: 40
        )
        ...
        with (
            patch.object(torch.cuda, "Event", side_effect=make_event),
            ...
            for generation in (1, 2):
                adapter.owner, adapter.owner_rid, adapter.generation = (
                    0,
                    str(generation),
                    generation,
                )
                full.k_buffer = [
                    torch.full((2056, 1, 256), generation, dtype=torch.uint8)
                ]
                full.v_buffer = [
                    torch.full((2056, 1, 256), generation + 17, dtype=torch.uint8)
                ]
    ...
    def test_light_observation_keeps_writeback_dependencies(self):
        for strict in (False, True):
            adapter = QSAHiSparseSingleRequest.__new__(QSAHiSparseSingleRequest)
            ...
            with (
                patch.object(torch.cuda, "Event", side_effect=events),
    ...
    def test_observation_rejects_invalid_or_contaminated_mode(self):
        ...
        for env, expected in (
            ...
            with (
                ...
                adapter.__init__(
                    SimpleNamespace(token_to_kv_pool=None, server_args=None), "offload"
                )
        ...
        for env in (
            ...
            with (
                ...
                QSAHiSparseSingleRequest(None, "offload")
    ...
    def test_layout_and_tail(self):
        ...
        for block in (0, 15, 16, 511):
            self.assertTrue(
                torch.equal(
                    packed[block, :1024], k[block * 4 : block * 4 + 4].flatten()
                )
            )
            self.assertTrue(
                torch.equal(
                    packed[block, 1024:], v[block * 4 : block * 4 + 4].flatten()
                )
            )
        got = (
            packed.view(-1, 256)
            .index_select(0, unpack_index("cpu"))
            .view(2, 2048, 1, 256)
        )
    ...
    def test_configuration(self):
        ...
        validate_configuration(
            graph_args, graph_pool, p2=True, graph=True, strict=False
        )
        ...
        validate_configuration(
            native_args, graph_pool, p2=True, graph=True, strict=False
        )
    ...
    def test_stale_release(self):
        ...
        allocator = PagedTokenToKVPoolAllocator(
            ...
            dtype=torch.uint8,
            device="cpu",
            kvcache=SimpleNamespace(qsa_hisparse=adapter),
            need_sort=True,
        )
        adapter.runner.token_to_kv_pool_allocator = allocator
        events = []
        adapter.record = lambda event: events.append(
            (event, allocator.available_size())
        )
