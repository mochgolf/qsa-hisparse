...
class TestQSAHiSparseRuntime(unittest.TestCase):
    ...
    def test_capture_environment_is_rejected(self):
        with (
            patch.dict(
                os.environ, {"SGLANG_QSA_HISPARSE_V3_CAPTURE": "/tmp/old-trace"}
            ),
            self.assertRaisesRegex(ValueError, "does not support V3 capture"),
        ):
            QSAHiSparseRuntime(None, "p2-offload")

    def test_graph_routes_events_and_failed_submission(self):
        from sglang.srt.model_executor.runner.shape_key import ShapeKey

        a = QSAHiSparseRuntime.__new__(QSAHiSparseRuntime)
        ...
        with (
            ...
            persistent = [
                ...
                *(
                    t
                    for s in a.layer_states
                    for n, t in s.items()
                    if n in ("hot", "tokens", "lru")
                ),
            ]
            ...
            with patch(
                ...
                with (
                    patch.object(
                        a.batch_write_locs,
                        "zero_",
                        side_effect=RuntimeError("reset fault"),
                    ),
            ...
            def batch(indices, seqs):
                return SimpleNamespace(
                    ...
                    req_pool_indices_cpu=torch.tensor(indices),
                    req_pool_indices=torch.tensor(indices),
                    seq_lens_cpu=torch.tensor(seqs),
                    seq_lens=torch.tensor(seqs),
                    rids=[a.requests[i].lease.rid for i in indices],
            ...
            with (
                self.assertRaisesRegex(RuntimeError, "aliases physical slots"),
            ...
            expected = torch.cat(
                (
                    state.full.k_buffer[0][1:5].flatten(),
            ...
            with (
                self.assertRaisesRegex(RuntimeError, "injected") as caught,
                a.graph_replay_scope(),
            ):
                ...
                with (
                    ...
                    patch.object(
                        a.producer_stream,
                        "synchronize",
                        side_effect=RuntimeError("producer drain"),
                    ),
                    patch.object(
                        a.copy_stream,
                        "synchronize",
                        side_effect=RuntimeError("copy drain"),
                    ) as copy_drain,
                ):
                    a.finish_graph_replay(1)
            ...
            with (
                patch.object(
                    a.copy_stream,
                    "synchronize",
                    side_effect=RuntimeError("still pending"),
                ),
    ...
    @unittest.skipUnless(
        os.environ.get("TRITON_INTERPRET") == "1", "explicit CPU interpreter run only"
    )
    def test_graph_byte_kernels_cpu_interpreter(self):
        from sglang.srt.layers.attention.qsa.hisparse_graph import (
            close_c4,
            finish_compact,
        )
        ...
        unpacked = torch.stack(
            [
                torch.stack((members ^ row, members ^ (137 + row)))
        ...
        for real_count in (0, batch):
            ...
            if real_count == 0:
                ...
                for row, slot in enumerate(slots.tolist()):
                    ...
                    self.assertTrue(
                        torch.equal(
                            compact[:, base + 1 : base + 1 + selected],
                            unpacked[row, :, :selected],
                        )
                    )
                    for plane, source in enumerate((k, v)):
                        ...
                        self.assertTrue(
                            torch.equal(
                                compact[
                                    plane,
                                    base + 1 + selected : base + 1 + selected + tail,
                                ],
                    ...
                    self.assertTrue(
                        torch.equal(
                            table[slot, raw[row, : selected + tail].long()],
                            torch.arange(
                                base + 1, base + 1 + selected + tail, dtype=torch.int32
                            ),
                        )
                    )
                    if tail == 0:
                        ring = start + slot * 5 + 1
                        expected = torch.cat(
                            (k[ring : ring + 4].flatten(), v[ring : ring + 4].flatten())
                        )
                        self.assertTrue(torch.equal(hot[slot * 2112 + 2048], expected))
                        self.assertEqual(
                            int(tokens[slot, 2048]), int(lengths[row]) // 4 - 1
                        )

    def test_light_decode_ledger_uses_only_host_schedule(self):
        adapter = QSAHiSparseRuntime.__new__(QSAHiSparseRuntime)
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch.object(
                    torch.cuda,
                    "memory_allocated",
                    side_effect=AssertionError("GPU query"),
                ),
                patch.object(
                    adapter,
                    "native_lease_snapshot",
                    side_effect=AssertionError("page query"),
                ),
            ):
                for event in ("decode_batch", "prefill_chunk"):
                    adapter.record(event, batch_size=1, rows=rows)
            records = [
                json.loads(line) for line in adapter.path.read_text().splitlines()
            ]
        self.assertEqual(
            [r["event"] for r in records], ["decode_batch", "prefill_chunk"]
        )
    ...
    def test_b8_workspace_row_routing_and_slot_reuse(self):
        ...
        a.graph_enabled, a.graph_capture_size, a.graph_batch = True, None, None
        a.graph_copy_pending, a.forward_id, a.offloaded = False, 0, False
        a.graph_failed_leases, a.requests, a.pending_releases, a.batch_requests = (
            set(),
            {},
            [],
            [],
        ...
        a.req_pool = SimpleNamespace(
            req_generation=torch.tensor([0] + [1] * 8),
            enable_mamba_extra_buffer=False,
            req_index_to_mamba_index_mapping=torch.arange(9),
        )
        a.host_slabs = torch.zeros(
            (a.max_requests, 1, a.capacity // 4, 2048), dtype=torch.uint8
        )
        ...
        with (
            patch.dict(os.environ, {"SGLANG_QSA_P2_VALIDATE_INITIAL_PAIR": "3"}),
        ...
        with (
            ...
            for idx in range(1, 9):
                ...
                if idx <= 4:
                    req = SimpleNamespace(
                        rid=str(idx), kv=SimpleNamespace(req_pool_idx=idx)
                    )
                    ...
                    if idx < 4:
                        ...
                        self.assertEqual(
                            [req.rid for req in released], ["1", "2", "3", "4"]
                        )
        ...
        batch = SimpleNamespace(
            ...
            seq_lens_cpu=torch.tensor(seqs),
            seq_lens=torch.tensor(seqs),
            rids=[str(i) for i in indices],
            forward_mode=SimpleNamespace(
                is_idle=lambda: False, is_decode=lambda: True, is_extend=lambda: False
            ),
        )
        a.begin_batch(batch)
    ...
    def test_ready_batch_preserves_position_metadata(self):
        ...
        reqs = [
            Req(str(i), "", array("q", range(n)), SamplingParams(max_new_tokens=4))
        ...
        def positions(batch, expected):
            ...
            self.assertTrue(
                torch.equal(forward.mrope_positions, torch.tensor([expected] * 3))
            )
    ...
    def test_batch_isolation_and_real_postflush(self):
        a = QSAHiSparseRuntime.__new__(QSAHiSparseRuntime)
        a.mode, a.device, a.rank, a.strict, a.observe = (
            "p2-offload",
            "cpu",
            0,
            True,
        ...
        logical = PagedTokenToKVPoolAllocator(
            2 * a.capacity, 64, a.pool.dtype, "cpu", a.pool, False
        )
        ...
        def claim(rid):
            ...
            return SimpleNamespace(
                rid=rid, kv=SimpleNamespace(req_pool_idx=req_idx)
            ), rows
        ...
        def resolver(*args):
            ...
            self.assertEqual(
                lengths.tolist(), [s.seq_len // 4 for s in a.batch_requests]
            )
            for row, slot in enumerate(slots.tolist()):
                ids = blocks[row].long()
                self.assertTrue(torch.equal(tokens[slot, ids], blocks[row]))
                self.assertTrue(
                    torch.equal(host[host_locs[slot, ids]], a.host_slabs[slot, 0, ids])
                )
        ...
        with (
            ...
            with (
                patch.object(
                    torch.distributed,
                    "all_reduce",
                    side_effect=lambda total, **kw: total.fill_(1),
                ),
                patch.object(
                    torch.distributed, "all_gather_object", side_effect=peer_nonempty
                ),
            ...
            for req, rows, length, kbyte, vbyte in ((req_a, ar, 2049, 7, 19),):
                ...
                with patch.object(
                    torch.distributed, "all_gather_object", side_effect=slow
                ):
            ...
            backend._store_kv(
                layer,
                br[:2050],
                torch.full((2050, 1, 256), 31, dtype=torch.uint8),
                torch.full((2050, 1, 256), 43, dtype=torch.uint8),
            )
            physical = a.prefill_slots(req_b.kv.req_pool_idx, 2050)
            self.assertTrue(
                torch.all(a.full.k_buffer[0].view(torch.uint8)[physical] == 31)
            )
            ...
            self.assertTrue(
                torch.equal(a.requests[req_a.kv.req_pool_idx].host, saved_a)
            )
            ...
            with (
                patch.object(
                    torch.distributed, "all_gather_object", side_effect=barrier_mismatch
                ),
                self.assertRaisesRegex(RuntimeError, "identities"),
            ):
                coord.collect_ready_reqs()
            ...
            with (
                patch.object(
                    torch.distributed, "all_gather_object", side_effect=slot_mismatch
                ),
                self.assertRaisesRegex(RuntimeError, "identities"),
            ):
                coord.collect_ready_reqs()
            ...
            self.assertEqual(
                native["logical_row_ptr"], a.req_table[req_b.kv.req_pool_idx].data_ptr()
            )
            ...
            a.req_pool.req_index_to_mamba_ping_pong_track_buffer_mapping = torch.tensor(
                [[0], [11], [13]]
            )
            a.req_pool.req_index_to_mamba_ping_pong_track_buffer_mapping[
                req_b.kv.req_pool_idx
            ] = 13
            self.assertEqual(
                a.native_lease_snapshot(state_b)["mamba_track_slots"], [13]
            )
            a.req_pool.enable_mamba_extra_buffer = False
            ...
            for seq_a, seq_b in (
                ...
                backend._store_kv(
                    layer,
                    loc,
                    torch.tensor([31, 7], dtype=torch.uint8)[:, None, None].expand(
                        2, 1, 256
                    ),
                    torch.tensor([43, 19], dtype=torch.uint8)[:, None, None].expand(
                        2, 1, 256
                    ),
                )
                a.after_store(layer)
                ...
                for seq in (seq_b, seq_a):
                    ...
                    topk.append(
                        torch.cat(
                            (
                                (ids[:, None] * 4 + torch.arange(4)).flatten(),
                                torch.arange(seq - tail, seq),
                                torch.full((3 - tail,), -1),
                            )
                        )
                    )
                ...
                for r, seq, kb, vb in ((0, seq_b, 31, 43), (1, seq_a, 7, 19)):
                    locs = table[
                        physical_rows[r], topk[r, : 2048 + seq % 4].long()
                    ].long()
            ...
            self.assertEqual(
                a.requests[req_c.kv.req_pool_idx].generation, lease_a.generation + 1
            )
            with self.assertRaisesRegex(RuntimeError, "stale"):
                a.after_release(lease_a)
            with self.assertRaisesRegex(RuntimeError, "stale"):
            ...
            backend._store_kv(
                layer,
                cr[:2048],
                torch.full((2048, 1, 256), 53, dtype=torch.uint8),
            ...
            self.assertEqual(
                a.requests[req_c.kv.req_pool_idx].states[0]["hot"].data_ptr(),
                inactive.data_ptr(),
            )
            self.assertTrue(torch.equal(state_b.states[0]["hot"], b_hot))
            ...
            with (
                patch.object(
                    a.slots, "commit_release", side_effect=RuntimeError("commit fault")
                ),
            ...
            with (
                patch.object(a, "record", side_effect=record_fault),
